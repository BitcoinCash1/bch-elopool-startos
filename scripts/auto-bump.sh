#!/usr/bin/env bash
# Bump the package to a new upstream EloPool (ckpool) release, commit it to
# master and release it.
#
...
# starts at package revision 0), resets ALLOW_DOWNGRADE to false, updates `ARG
# CKPOOL_REF` in the Dockerfile, then commits the bump to master; Check Upstream
# then dispatches Tag and Release. Check that every patch in patches/apply.py
# still applies to the new source first.
#
