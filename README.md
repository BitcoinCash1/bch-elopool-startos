...
- **Rejected** — rejected shares. ckpool only keeps pool-wide reject telemetry, so this will usually read 0 per-worker. If you need fine-grained rejection stats (stale, HW error, low-diff) consult your ASIC's cgminer API on port 4028.
