...
- *No "Rejected" column.* ckpool only accounts rejects pool-wide (`stats.accounted_rejects`) — `stratifier.c` keeps no per-client reject counter, so a per-worker Rejected column would always read 0 and be misleading. For real reject telemetry (stale / HW error / low-diff) query your ASIC's own cgminer API on port 4028.
